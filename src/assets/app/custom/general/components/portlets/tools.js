...
var KTPortletTools = {
	init: function() {
		...
		(toastr.options.showDuration = 1e3),
			(e = new KTPortlet('kt_portlet_tools_1')).on('beforeCollapse', function(e) {
				setTimeout(function() {
			...
			e.on('afterFullscreenOff', function(e) {
				...
				toastr.warning('After fullscreen off event fired!'),
					(t = $(e.getBody()).find('> .kt-scroll')) &&
						((t = $(e.getBody()).find('> .kt-scroll')).css('height', t.data('original-height')),
			...
			(function() {
				...
				e.on('beforeCollapse', function(e) {
					...
					e.on('reload', function(e) {
						toastr.info('Leload event fired!'),
							KTApp.block(e.getSelf(), { type: 'loader', state: 'success', message: 'Please wait...' }),
							setTimeout(function() {
								KTApp.unblock(e.getSelf());
							}, 2e3);
					}),
			...
			(function() {
				...
				e.on('beforeCollapse', function(e) {
					...
					e.on('reload', function(e) {
						toastr.info('Leload event fired!'),
							KTApp.block(e.getSelf(), { type: 'loader', state: 'brand', message: 'Please wait...' }),
							setTimeout(function() {
								KTApp.unblock(e.getSelf());
							}, 2e3);
					}),
